...
export default defineConfig({
  ...
  use: {
    ...
    // Force utilization of host Google Chrome to bypass offline browser installation issues
    channel: 'chrome', 
    headless: true,
  },
  projects: [
...
| ID | Test Case Description | Expected Result |
|---|---|---|
| T4-1 | Complete desktop flow | User enters `/library`, searches for "Next.js", clicks the book, opens reader, zooms in, clicks download, and verifies download content. |
| T4-2 | Complete mobile flow | Under iPhone viewport, user filters by category, taps book card, reads in mobile responsive reader, navigates pages, and downloads PDF locally. |
