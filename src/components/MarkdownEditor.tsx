...
export default function MarkdownEditor() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(err);
      const msg = err?.message || 'تعذر الاتصال بالسيرفر';
      setSaveStatus('❌ فشل الحفظ: ' + msg);
    } finally {
      setSaving(false);
...
OUTPUT: Return ONLY the complete operational Markdown Skill file starting with YAML frontmatter. No preamble or chat.`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'فشل التوليد - تأكد من مفاتيح API في الإعدادات');
      }

      if (!response.body) throw new Error('لم يتم استلام استجابة من السيرفر');
      ...
      console.error(err);
      const msg = err?.message || 'تعذر الاتصال بخدمة التوليد';
      setValue(prev => prev + '\n\n❌ حدث خطأ أثناء التوليد: ' + msg);
    } finally {
  ...
  return (
    <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 40px)', margin: '20px 0', width: '100%' }}>
      {/* Header */}
      <div style={{ 
        display: 'flex', 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: '280px' }}>
          ...
          <ChevronLeft size={16} color="var(--text-muted)" className="rtl:rotate-180" />
          <input
            type="text"
            value={filename}
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {isFork && value !== originalValue && (
            <span style={{ fontSize: '0.8rem', color: 'var(--brand-primary)', background: 'var(--bg-surface-hover)', border: '1px solid var(--border-subtle)', padding: '4px 10px', borderRadius: '6px' }}>
          ...
          <div style={{ display: 'flex', border: '1px solid var(--border-subtle)', borderRadius: '8px', overflow: 'hidden' }}>
            <button
              ...
              style={{ background: 'var(--bg-surface-solid)', border: 'none', padding: '8px 12px', cursor: 'pointer', color: 'var(--text-main)', borderInlineEnd: '1px solid var(--border-subtle)' }}
              title="تراجع (Ctrl+Z)"
