...
- Name: scenarios.spec.ts >> Tier 4: Real-World Scenarios >> T4-4: UI displays fallback error gracefully on API/R2 failure and user can retry
- Location: tests\scenarios.spec.ts:223:7
...
> 233 |     await page.goto('/library');
  ...
  283 | 
  284 |     // Grid should recover and display 45 books, and search input should be empty
  285 |     await expect(searchInput).toHaveValue('');
  286 |     const bookCards = page.locator('[data-testid="book-card"]');
  287 |     await expect(bookCards).toHaveCount(45);
  288 |   });
  289 | });
  290 | 
```